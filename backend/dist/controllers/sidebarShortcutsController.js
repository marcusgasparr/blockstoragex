...
exports.SidebarShortcutsController = void 0;
const SidebarShortcut_1 = require("../lib/models/SidebarShortcut");
class SidebarShortcutsController {
    /**
     * GET /api/sidebar-shortcuts/user/:userId
     * Buscar todos os atalhos de um usuário
     */
    static async getUserShortcuts(req, res) {
        try {
            ...
            const shortcuts = await SidebarShortcut_1.SidebarShortcut.getUserShortcuts(userId);
            res.json({
                success: true,
                data: shortcuts
            });
    ...
    static async createShortcut(req, res) {
        try {
            ...
            const sort_order = await SidebarShortcut_1.SidebarShortcut.getNextSortOrder(user_id);
            const shortcutData = {
                user_id,
            ...
            const shortcutId = await SidebarShortcut_1.SidebarShortcut.create(shortcutData);
            res.status(201).json({
                success: true,
                data: {
                    id: shortcutId,
    ...
    static async updateShortcut(req, res) {
        try {
            ...
            const belongsToUser = await SidebarShortcut_1.SidebarShortcut.belongsToUser(shortcutId, user_id);
            if (!belongsToUser) {
            ...
            const updated = await SidebarShortcut_1.SidebarShortcut.update(shortcutId, user_id, updateData);
            if (!updated) {
                return res.status(404).json({
                    success: false,
    ...
    static async deleteShortcut(req, res) {
        try {
            ...
            const deleted = await SidebarShortcut_1.SidebarShortcut.delete(shortcutId, user_id);
            if (!deleted) {
                return res.status(404).json({
                    success: false,
                    message: 'Atalho não encontrado'
     ...
     * Reordenar atalhos
     */
    static async reorderShortcuts(req, res) {
        try {
            ...
            await SidebarShortcut_1.SidebarShortcut.reorder(user_id, shortcut_ids);
            res.json({
                success: true,
...
exports.SidebarShortcutsController = SidebarShortcutsController;
